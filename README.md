...
(4) 英文的变量名、类型名、函数名、值和中文之间要有一个空格。<br>
(5) 请使用UTF-8编码进行注释。<br>
### 注释建议：
...
(2) 本仓库只增加和修改中文注释，同时为了注释可以增加空格和空行！注意不要修改源码，不要破坏源码的结构！如果你真的对源码有修改的想法，请到redis仓库发起pull request。还有如果你发现了英文注释有误，你想对英文注释进行修改，我们经过审核后可以在这边先进行合并，但同时你也别忘了到redis的仓库提一下pr告知官方哦！🥇🥇<br>
(3) 对于commit信息，请写清楚你对哪个函数进行了注释的添加或修改。如：“添加setGenericCommand函数的注释”，“修正setGenericCommand函数注释的格式错误”。
## FAQ:
### Q: 该项目是定格在redis7.0版本了吗？还是会更新呢？
A: 项目暂不确定会不会跟着redis更新，更新可能会出现被注释过的代码可能会跟最新代码不一致的问题，需要慎重考虑，当然我们还是希望这个仓库是能跟着更新的，也许1个月更新一次？🤔🤔
